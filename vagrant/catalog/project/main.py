...
def newMenuItem(restaurant_id):
    ...
    if request.method == 'POST':
        newItem = MenuItem(name=request.form[
                           'name'], course=request.form['course'], description=request.form['description'], price=request.form['price'], restaurant_id=restaurant_id, user_id=login_session['user_id'])
        session.add(newItem)
        session.commit()
...
def editMenuItem(restaurant_id, menu_id):
    ...
    if itemToEdit.user_id != login_session['user_id']:
        return "<script>function myFunction() {alert('You are not authorized to edit items from this restaurant's menu. Please create your own restaurant in order to edit its menu.');}</script><body onload='myFunction()''>"
    if request.method == 'POST':
        if request.form['name']:
...
def deleteMenuItem(restaurant_id, menu_id):
    ...
    if itemToDelete.user_id != login_session['user_id']:
        return "<script>function myFunction() {alert('You are not authorized to delete items from this restaurant's menu. Please create your own restaurant in order to delete its menu.');}</script><body onload='myFunction()''>"
    if request.method == 'POST':
